...
4. **合作伙伴** — 登记合作伙伴（渠道商 / 代理商等），可随时为其提交备注记录（不占用签约、金额、跟进时间字段，备注按系统时间归档）。
5. **材料库** — 上传产品方案、报价单、合同模板等资料（PDF / PPT / Word / Excel / HTML / 图片 / 压缩包等），支持多文件批量上传、在线预览（PDF/图片）、下载与删除。
...
- **续约跟进（Renew）**：到期少于45天的在约客户会自动生成 Renew 跟进；该面板顶部提供「＋ 新建 Renew 客户」，可直接录入续约机会（客户名称、预计金额、续约到期时间、跟进日期等）并纳入续约跟进列表，续约完成后自动退出；手动新建的 Renew 客户不会被同步逻辑自动删除。
- **合作伙伴板块**：独立于销售管线的合作伙伴登记表（渠道商 / 代理商等），只保留名称与联系人，**不包含金额、跟进时间、订阅签约等字段**；可新建 / 编辑 / 删除，并在详情弹窗内为其**提交备注**，备注与其他客户一样按系统时间归档到时间线（支持逐条删除）。
